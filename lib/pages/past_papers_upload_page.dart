...
class _PastPapersUploadPageState extends State<PastPapersUploadPage> {
  ...
  Future<void> _pickFile() async {
    final result = await FilePicker.platform.pickFiles(
      type: FileType.custom,
      allowedExtensions: ['pdf'],
      withData: true, // Necessary for Web and bytes
    );
  ...
  void _uploadPaper() async {
    if (!_formKey.currentState!.validate() || _selectedFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a PDF and fill all fields.'),
        ),
      );
      return;
    }
          ...
          child: Column(
            children: [
              ElevatedButton.icon(
                ...
                icon: const Icon(Icons.picture_as_pdf),
                label: Text(
                  _selectedFile == null ? 'Select PDF' : 'Change File',
                ),
                style: ElevatedButton.styleFrom(
              ...
              if (_selectedFile != null) Text('File: ${_selectedFile!.name}'),
              const SizedBox(height: 20),
              TextFormField(
